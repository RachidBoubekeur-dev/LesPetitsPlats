...
const dataFactory = () => {
    ...
    const searchRecettes = (search) => {
        const searchByName = arrayRecettes.filter(recette => {
            return recette.name.toLowerCase().includes(search);
        });

        const searchByIngredients = arrayRecettes.filter(recette => {
        ...
        const searchByDescription = arrayRecettes.filter(recette => {
            return recette.description.toLowerCase().includes(search);
        });

        if (searchByName.length >= (searchByIngredients.length && searchByDescription.length)) return searchByName;
        else if (searchByIngredients.length >= (searchByName.length && searchByDescription.length)) return searchByIngredients;
        else return searchByDescription;
    };
